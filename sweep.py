...
def get_args():
    ...
    parser.add_argument(
        "-p", "--prefix", required=True, help="save checkpoints and logs in <checkpoints-dir>/<prefix>.<save_dir_key>"
    )
    parser.add_argument(
        "-t",
        "--num-trials",
    ...
    parser.add_argument(
        "--checkpoints-dir",
        default=os.path.join("/checkpoint02", os.environ["USER"], str(datetime.date.today())),
        help="save checkpoints and logs in <checkpoints-dir>/<prefix>.<save_dir_key>",
    )
    parser.add_argument(
        "--todo-dir",
        default=os.path.join("/checkpoint02", os.environ["USER"], "TODO"),
        help="save symlinks to runs in <todo-dir>/<prefix>.<save_dir_key>",
    )
    parser.add_argument(
        "--resume-failed",
        action="store_true",
    ...
    parser.add_argument(
        "--resume-finished", action="store_true", help="force any runs that finished to begin again (uncommon)"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="output only a list of actions to perform without performing them"
    )
    parser.add_argument("--local", action="store_true", help="run locally instead of submitting remote job")
    parser.add_argument("--todo", action="store_true", help="add symlink in TODO directory")
    parser.add_argument(
        "--partition",
        default="uninterrupted",
        choices=["priority", "learnfair", "uninterrupted", "scavenge"],
        help="SLURM partition",
    )
    parser.add_argument("--exclusive", action="store_true")
    parser.add_argument("--dep", type=int)
    parser.add_argument("--sequential", action="store_true", help="run jobs sequentially")
...
def launch_train(args, config):
    ...
    # compute save_dir
    save_dir_key = ".".join(
        filter(lambda save_dir_key: save_dir_key is not None, [hp.get_save_dir_key() for hp in config.values()])
    )
    ...
    todo_dir = os.path.join(args.todo_dir, f"{args.prefix}.{save_dir_key}.ngpu{num_total_gpus}")

    # create save directory if it doesn't exist
    if not os.path.exists(save_dir):
        if not dry_run(f"create directory: {save_dir}"):
            if args.todo and os.path.exists(todo_dir):
                raise Exception(f"TODO sym link already exists: {todo_dir}")
            os.makedirs(save_dir)
            if args.todo:
                os.symlink(save_dir, todo_dir)
    ...
    if args.num_nodes > 1:
        # train_cmd.extend(['--distributed-init-method', 'slurm'])
    ...
    if args.local:
        ...
        sbatch_cmd = [
            ...
            str(ntasks_per_node),
            "--cpus-per-task",
            str(int(8 * args.num_gpus / ntasks_per_node)),
            "--mem-per-cpu",
            "6G",
            "--output",
            train_log,
